...
def classify_pdb(row):
    qtm = fnum(row.get("qtmscore", ""))
    prob = fnum(row.get("prob", ""))
    qcov = fnum(row.get("qcov", ""))

    if qtm >= 0.8 and prob >= 0.9 and qcov >= 0.7:
        return "STRONG_PDB_STRUCT_MATCH", "High query TM-score, high probability, and broad query coverage"
    if qtm >= 0.5 and prob >= 0.9 and qcov >= 0.5:
        return "MODERATE_PDB_STRUCT_MATCH", "Moderate query TM-score with high probability and sufficient query coverage"
    if qtm >= 0.4 and prob >= 0.9:
        return "DOMAIN_LEVEL_OR_PARTIAL_PDB_MATCH", "Possible partial/domain-level match; inspect domain boundaries and coverage"
    return "WEAK_OR_NO_PDB_STRUCT_MATCH", "No confident PDB structural match under current criteria; consider AF/Swiss-Prot or family-specific reference search"
...
classified_fields = ["protein_id","family","habitat"] + hit_fields + ["pdb_struct_support_class","pdb_struct_support_note"]
top5_fields = ["protein_id","family","habitat","rank"] + hit_fields + ["pdb_struct_support_class","pdb_struct_support_note"]
audit_fields = [
...
for q in query_order:
    ...
    rank1 = hs[0]
    rank1_class, rank1_note = classify_pdb(rank1)

    qtmhit = max(hs, key=lambda r: (fnum(r.get("qtmscore", "")), fnum(r.get("prob", "")), fnum(r.get("qcov", "")), fnum(r.get("bits", ""))))
    qtm_class, qtm_note = classify_pdb(qtmhit)

    r1 = dict(base)
    r1.update(rank1)
    r1["pdb_struct_support_class"] = rank1_class
    r1["pdb_struct_support_note"] = rank1_note
    rank1_rows.append(r1)

    qm = dict(base)
    qm.update(qtmhit)
    qm["pdb_struct_support_class"] = qtm_class
    qm["pdb_struct_support_note"] = qtm_note
    qtm_rows.append(qm)

    for idx, h in enumerate(hs[:5], start=1):
        cls, note = classify_pdb(h)
        tr = dict(base)
        tr["rank"] = idx
        tr.update(h)
        tr["pdb_struct_support_class"] = cls
        tr["pdb_struct_support_note"] = note
        top5_rows.append(tr)
    ...
    for idx, h in enumerate(hs[1:5], start=2):
        alt_class, alt_note = classify_pdb(h)
...
class_counts = {}
for r in rank1_rows:
    cls = r.get("pdb_struct_support_class", "")
...
with pointer_path.open("w") as f:
    ...
    w.writerow(["pdb_primary_besthit_classified", str(primary_classified), "PRIMARY downstream input for P2Rank/reference/overlay/decision matrix"])
    w.writerow(["pdb_primary_besthit_rank1", str(primary_best), "Raw rank-1 Foldseek best hit table"])
    w.writerow(["pdb_top5_hits", str(top5_table), "Top-5 audit/evidence table, not primary downstream input"])
    w.writerow(["pdb_qtmmax_audit", str(qtm_max), "Secondary qTM-max audit table only"])
    w.writerow(["pdb_rank1_vs_qtmmax_audit", str(rank_audit), "Manual review flags for rank1 vs qTM-max differences"])
    w.writerow(["pdb_neartie_top5_audit", str(neartie_audit), "Top-5 near-tie and alternative-support audit"])
    w.writerow(["pdb_canonical_summary", str(summary_path), "Canonical PDB Foldseek summary"])
